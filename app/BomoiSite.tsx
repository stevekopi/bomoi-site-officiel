...
function HomePage() {
  return <>
    <section className="hero"><div className="hero-grid"><div className="hero-copy"><span className="eyebrow">ERP MODERNE · MULTISITE</span><h1>Pilotez toute votre entreprise, <span>simplement.</span></h1><p>Bomoi centralise vos stocks, ventes, achats, finances et opérations multisites dans une plateforme claire, rapide et sécurisée.</p><div className="hero-actions"><a className="btn" href="/demonstration">Demander une démonstration</a><a className="btn secondary" href="/modules">Découvrir les modules</a></div><div className="trust-row"><span>◉ <b>Sécurisé</b></span><span>ϟ <b>Rapide</b></span><span>▦ <b>Multisite</b></span><span>☁ <b>Accessible</b></span></div></div><Dashboard /></div></section>
    <section className="logos-strip"><span>ILS NOUS FONT CONFIANCE</span><div>{["PARTENAIRE 01", "RÉFÉRENCE 02", "PARTENAIRE 03", "RÉFÉRENCE 04"].map(x=><b key={x}>{x}</b>)}</div><small>Emplacements réservés aux logos officiels</small></section>
...
const plans = [
  ...
  { name: "Duo", users: "2 utilisateurs", price: "60", desc: "Parfait pour les petites équipes en croissance." },
  { name: "Team", users: "5 utilisateurs", price: "120", desc: "Pour les équipes souhaitant plus de collaboration." },
  { name: "Business", users: "10 utilisateurs", price: "230", desc: "Tout ce qu’il vous faut pour une gestion performante.", popular: true },
  { name: "Enterprise", users: "50 utilisateurs", price: "1000", desc: "Conçu pour les entreprises de grande taille." },
...
function NewsPreview() { return <section className="section news-section"><div className="section-heading"><div><span className="eyebrow">ACTUALITÉS</span><h2>Les dernières évolutions.</h2></div><a className="text-link" href="/actualites">Toutes les actualités →</a></div><div className="news-grid">{news.map(n=><article key={n.title}><div className="news-image"><span>Visuel à fournir</span></div><small>{n.tag} · {n.date}</small><h3>{n.title}</h3><p>{n.text}</p><a href="/actualites">Lire l’article →</a></article>)}</div></section>; }
function NewsPage() { return <><PageHero eyebrow="ACTUALITÉS" title="La vie et les évolutions de Bomoi." intro="Retrouvez les nouveautés produit, conseils pratiques et informations de l’écosystème Bomoi."/><section className="section news-grid all">{[...news,...news.map((n,i)=>({...n,title:["Mieux piloter ses tournées de livraison","Comprendre le stock disponible","Les tableaux de bord au service des décisions"][i],tag:"Conseils"}))].map((n,i)=><article key={i}><div className="news-image"><span>Visuel éditorial {i+1}</span></div><small>{n.tag} · {n.date}</small><h3>{n.title}</h3><p>{n.text}</p><a href="#">Lire l’article →</a></article>)}</section></>; }
